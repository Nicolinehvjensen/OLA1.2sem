...
test <- unique(f_shot_pass_polish$player.id) #715 spillere
t <- unique(f_shot_pass_dutch$player.id) #712 spillere

pass_data <- data.frame(cbind(f_shot_pass_dutch$location.x, f_shot_pass_dutch$location.y, f_shot_pass_dutch$pass.angle, f_shot_pass_dutch$pass.length, f_shot_pass_dutch$pass.endLocation.x, f_shot_pass_dutch$pass.endLocation.y))
# tilføj player.id
player.id <- as.numeric(f_shot_pass_dutch$player.id)
player.name <- f_shot_pass_dutch$player.name
player.position <- f_shot_pass_dutch$player.position
...
set.seed(123)
k <- 4
kmeans_result_pass_dutch <- kmeans(pass_data[1:6], centers = k, nstart = 20)
pass_data1 <- data.frame(cbind(pass_data, kmeans_result_pass_dutch$cluster))

# Beregn WSS for forskellige værdier af k
wss <- sapply(1:7, function(k) kmeans(pass_data[1:6], centers = k, nstart = 20)$tot.withinss)
...
ggplot(long_data, aes(x = clusters, y = value, fill = variable)) +
  ...
  labs(x = "Cluster", y = "Værdi", title = "Cluster 1 + 4 har lange afleveringer") +
  theme_minimal()+
...
table_positions_c1 <- data.frame(table(cluster1$player.position))
...
generate_cluster_plot <- function(cluster_data, cluster_name) {
  ...
  top_10_players <- cluster_data %>%
    arrange(desc(percentage)) %>%
    slice_head(n = 10)
  
  # Opret ggplot 
  plot <- ggplot(top_10_players, aes(x = reorder(player.name, percentage), y = percentage, fill = cluster_name)) +
    geom_bar(stat = "identity", position = "dodge", width = 0.7) +
    labs(x = "", y = "Procentandel", fill = cluster_name, 
         title = paste("Top 10 spillere i", cluster_name)) +
    coord_flip() +
    theme_minimal() +
    theme(axis.text.x = element_text(size = 12, face = "bold"),
...
# afprøv metode 2 - unik spiller

un_p_pass <- f_shot_pass_dutch %>% 
  group_by(player.id, player.name) %>% 
  filter(n() > 100) %>%
  summarise(
...
un_p_pass_sd <- f_shot_pass_dutch %>%
  group_by(player.id, player.name) %>%
  filter(n() > 100) %>%  # Filtrér grupper med færre end 2 observationer
  summarise(
...
un_p_pass_sd_norm <- un_p_pass_sd[3:8] %>%
  mutate(
...
summary(un_p_pass_sd_norm)

# Beregn standardafvigelsen for hver numerisk kolonne i datasættet
sd_values <- sapply(un_p_pass_sd_norm, function(x) if(is.numeric(x)) sd(x, na.rm = TRUE) else NA)
...
scale_data <- data.frame(scale(un_p_pass_sd_norm))

k <- 4
kmeans_result_un_p <- kmeans(scale_data, centers = k, nstart = 20)
un_p_pass <- cbind(un_p_pass, kmeans_result_un_p$cluster)
colnames(un_p_pass) <- c("player.id","player.name","avg_pass_angle","avg_pass_length","avg_x_location","avg_y_location","avg_end_x_location","avg_end_y_location", "clusters")
summary(un_p_pass)

pass_data_un_p <- un_p_pass %>% 
  group_by(clusters) %>% 
  summarise(
...
pass_data_un_p1 <- as.data.frame(pass_data_un_p[c(1,4,5)])
colnames(pass_data_un_p1) <- c("clusters", "pass.angle", "pass.length")
#klargør til plot
long_data_un_p <- pass_data_un_p1 %>% 
  pivot_longer(cols = c(pass.angle, pass.length), names_to = "variable", values_to = "value")

ggplot(long_data_un_p, aes(x = clusters, y = value, fill = variable)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  facet_wrap(~ variable, scales = "free_y", 
             labeller = labeller(variable = c(
               pass.angle = "Afleveringsvinkel i %",
  ...
  labs(x = "Cluster", y = "Værdi", title = "Cluster 2 har lange afleveringer") +
  theme_minimal()+
  theme(axis.text.x = element_text(size = 12, face = "bold"),
        plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
        strip.text = element_text(size = 14))
