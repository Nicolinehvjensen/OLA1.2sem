...
gamesdataflat <- games_data %>%
  ...
  select(-team, -player, -pass)  # Fjern de nested dataframes
  ...
  observeEvent(input$compare, { 
    ...
    output$barPlot <- renderPlot({
      ...
      pass_summary <- filtered_data %>%
        ...
        mutate(percentage = vals / total * 100)  # Beregner procentandel
      ...
      ggplot(pass_summary, aes(x = team_name, y = vals, fill = pass_type)) +
        geom_bar(stat = "identity", position = "stack") +  # Stacket søjlediagram
        geom_text(aes(label = sprintf("%.0f %%", percentage), y = y_position), 
                  color = "black", size = 6, fontface = "bold") +  # Procentlabels
        ...
        theme_minimal() +
        theme(
          plot.background = element_rect(fill = "white", color = NA),  # Hvid baggrund
          panel.background = element_rect(fill = "white", color = NA),
          text = element_text(color = "black"),  # Sort tekst
          axis.text = element_text(color = "black"),
          axis.title = element_text(color = "black", face = "bold"),
          plot.title = element_text(size = 18, face = "bold", color = "black"),
          legend.background = element_rect(fill = "white"),  # Hvid legend-baggrund
          legend.key = element_rect(fill = "white"),
          legend.title = element_text(color = "black", face = "bold"),
          legend.text = element_text(color = "black")
        )
    })
  })  # <--- DENNE MANGLEDE OG ER NU TILFØJET!!
}
